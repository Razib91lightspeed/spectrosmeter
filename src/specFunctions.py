...
def readcal(width):
    ...
    if len(pixels) == 3:
        ...
        wavelengthData = [round(coefficients(pixel), 6) for pixel in range(width)]
    elif len(pixels) > 3:
        coefficients = np.poly1d(np.polyfit(pixels, wavelengths, 3))
        wavelengthData = [round(coefficients(pixel), 6) for pixel in range(width)]

    return [wavelengthData, "Calibration Status", "Calibration Details", "Calibration Notes"]
...
def writecal(clickArray):
    ...
    with open('caldata.txt', 'w') as f:
        f.write(','.join(map(str, pxdata)) + '\r\n')
        f.write(','.join(map(str, wldata)) + '\r\n')
...
def generateGraticule(wavelengthData):
    """
    Generate graticule data for the spectrograph.
    """
    ...
    fifties = [[i, int(wavelengthData[i])] for i in range(len(wavelengthData)) if int(wavelengthData[i]) % 50 == 0]
